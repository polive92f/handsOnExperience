...
public class WalletServiceImpl implements WalletService {
	...
	public Boolean login(Integer walletId, String password) throws WalletException {
		...
		Wallet newWallet = this.walletRepository.getWalletById(walletId);
		boolean loginService=false;
		if (newWallet.getPassword().equals(password)) {
			 loginService = true;
		}
		return loginService;
	...
	public Double addFundsToWallet(Integer walletId, Double amount) throws WalletException {
		...
		Wallet newWallet = this.walletRepository.updateWallet(updateWallet);
